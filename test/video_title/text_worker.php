...
class text_worker {
	...
	function __construct(int $w=NULL, int $h=NULL, string $t=NULL, string $fontfile=NULL, string $result_file_path=NULL){
		...
		$this->image_put_text();
		$this->image_output($result_file_path);
	}
	...
	function calculate_font_size(){
		...
		throw new \Exception("text_worker: maybe too long text...",0);
	}
	...
	function image_output(string $result_file_path=NULL){
		imagejpeg($this->image, $result_file_path, 75);
	}
