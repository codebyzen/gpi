...
class video_worker {
	...
	function init(array $options=[]) {
		...
		foreach($this->opts['layers'] as $k=>$v) {
			...
			if ($v->type=='video') {
				$v->duration = $this->get_file_info($v->path)['duration'];
		...
		if (isset($this->opts['audio'])) {
			$this->opts['audio']->duration = $this->get_file_info($this->opts['audio']->path)['duration'];
		}
	...
	public function resizeto_new($video_layer_key,$fixed_width) {
		...
		if ($w/$h>$this->opts['destination']['MAX_RATIO']) { // width
			$back = [$fixed_width, $this->fix2div($fixed_width/$this->opts['destination']['RECOMMENDED_RATIO'])];
			$media = [$back[0], $this->fix2div($h/($w/$back[0]))];
		} elseif($w/$h<$this->opts['destination']['MIN_RATIO']) { // tall
			$back = [$fixed_width, $this->fix2div($fixed_width/$this->opts['destination']['RECOMMENDED_RATIO'])];
	...
	private function compile_filters(){
		...
		$textbox_height = 0;
		if ($text_key!==false) {
			$textbox_height = $this->fix2div($full_dim[1]/100*20);
			$full_dim[1] = $this->fix2div($full_dim[1]+$textbox_height);
		}
